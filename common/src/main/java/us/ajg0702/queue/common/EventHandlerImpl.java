...
public class EventHandlerImpl implements EventHandler {
    ...
    public void handleMessage(AdaptedPlayer receivingPlayer, byte[] data) {
        ...
        try {
            ...
            switch (subchannel) {
                case "ack" -> {
                    main.getPlatformMethods().sendPluginMessage(receivingPlayer, "ack", "yes, im here");
                }
                case "queue" -> {
                    String rawData = in.readUTF();
                    moveCommand.execute(new PlayerSender(receivingPlayer), new String[]{rawData});
                }
                case "cqueue" -> {
                    String rawData = in.readUTF();
                    main.getQueueManager().addToQueue(receivingPlayer, rawData);
                }
                case "massqueue" -> {
                    Arrays.stream(in.readUTF().split(",")).map(item -> item.split(":"))
                        .filter(pair -> pair.length == 2).forEach(pair -> {
                            AdaptedPlayer player = main.getPlatformMethods().getPlayer(pair[0]);
                            moveCommand.execute(new PlayerSender(player), new String[]{pair[1]});
                        });
...
//                        AdaptedPlayer player = main.getPlatformMethods().getPlayer(pname);
//                        moveCommand.execute(new PlayerSender(player), new String[]{ pserver });
//                    }
                }
                case "queuename" -> {
                    QueueServer server = main.getQueueManager().getSingleServer(receivingPlayer);
                    String name = server == null
                        ? main.getMessages().getString("plcaeholders.position.none")
                        : server.getAlias();

                    main.getPlatformMethods().sendPluginMessage(receivingPlayer, "queuename", name);
                }
                case "position" -> {
                    QueueServer server = main.getQueueManager().getSingleServer(receivingPlayer);
                    String pos = server == null
                        ? main.getMessages().getString("placeholders.position.none")
                        : server.getQueue().indexOf(server.findPlayer(receivingPlayer)) + 1 + "";

                    main.getPlatformMethods().sendPluginMessage(receivingPlayer, "position", pos);
                }
                case "positionof" -> {
                    QueueServer server = main.getQueueManager().getSingleServer(receivingPlayer);
                    String pos = server == null
                        ? main.getMessages().getString("placeholders.position.none")
                        : server.getQueue().size() + "";

                    main.getPlatformMethods().sendPluginMessage(receivingPlayer, "positionof", pos);
                }
                case "estimated_time" -> {
                    QueueServer server = main.getQueueManager().getSingleServer(receivingPlayer);
                    ...
                    String timeString;
                    if (server != null) {
                        QueuePlayer queuePlayer = server.findPlayer(receivingPlayer);
                    ...
                    main.getPlatformMethods().sendPluginMessage(receivingPlayer, "estimated_time", timeString);
                }
                case "inqueue" -> {
                    QueueServer server = main.getQueueManager().getSingleServer(receivingPlayer);
                    main.getPlatformMethods().sendPluginMessage(receivingPlayer, "inqueue", (server != null) + "");
                }
                case "queuedfor" -> {
                    ...
                    main.getPlatformMethods().sendPluginMessage(receivingPlayer, "queuedfor", srv, server.getQueue().size() + "");
                }
                case "leavequeue" -> {
                    ...
                    leaveCommand.execute(new PlayerSender(receivingPlayer), args);
                }
            }
        } catch (IOException e1) {
            main.getLogger().warning("An error occurred while reading data from spigot side:");
            e1.printStackTrace();
        }
    }
    ...
    public void onServerKick(AdaptedPlayer player, @NotNull AdaptedServer from, Component reason, boolean moving) {
        ...
        for (QueueServer server : queuedServers) {
            if (!(server.getServerNames().contains(from.getName()))) {
                continue;
            }
            QueuePlayer queuePlayer = server.findPlayer(player);
            if (queuePlayer.getPosition() != 1) {
                continue;
            }
